...
This document consolidates all top-level project documentation into a single, easy-to-navigate reference and adds a practical guide for generating and organizing SVG shape stimuli. All relevant information is available below in this file.
...
## Overview
For the full overview and feature set, see the [Overview](#overview) section below.
For more details (requirements, SSL setup, troubleshooting), see the [System Requirements](#system-requirements) and [Troubleshooting](#troubleshooting) sections below.
...
Run the Node HTTPS server (serves static files and API endpoints)

```bash
# from project root
npm install
# Starts an HTTPS Express server that serves the project and the API on https://localhost:8000
npm run serve-with-api-https
```

Windows:

- PowerShell: `./runexperiment.ps1`  
   (Run in PowerShell: `./runexperiment.ps1` or `powershell -ExecutionPolicy Bypass -File .\runexperiment.ps1`)
- Command Prompt: `runexperiment.bat`
...
The experiment requires HTTPS for security and webcam access. Two recommended approaches are shown below:

1) Recommended — use `mkcert` to create locally-trusted certificates (easiest for modern browsers).
2) Fallback — generate a manual self-signed certificate with `openssl`.

Using `mkcert` (recommended)

`mkcert` creates a local certificate authority and issues browser-trusted certs for localhost. Steps by OS:
...
- Windows (Chocolatey example / PowerShell):

```powershell
choco install mkcert -y
mkcert -install
mkcert localhost 127.0.0.1 ::1
# Rename/move produced files to the project root as cert.pem and key.pem
Move-Item .\localhost+2.pem .\cert.pem
Move-Item .\localhost+2-key.pem .\key.pem
```

Note: `mkcert` makes certificates trusted by the OS/browser and avoids manual CA import.

Using `openssl` (fallback)

If you prefer not to use `mkcert`, generate a self-signed certificate with `openssl` (may require importing the cert into your browser for full trust):

```bash
openssl req -newkey rsa:2048 -new -nodes -x509 -days 3650 -keyout key.pem -out cert.pem
```

Place `cert.pem` and `key.pem` in the project root directory. Browsers may warn unless the CA is trusted.

---

OS-specific Node.js (LTS) installation and npm

We recommend using the latest Node.js LTS release for `npm` and tools like `http-server`.

- Linux / macOS (recommended: `nvm`):

```bash
curl -fsSL https://raw.githubusercontent.com/nvm-sh/nvm/v0.39.5/install.sh | bash
# follow the printed instructions to load nvm in your shell, then:
nvm install --lts
node -v
npm -v
```
...
npx http-server -S -C cert.pem -K key.pem -p 8888 StudentFolders/Experiment/Experiment
```
...
Option A — official installer (recommended for most users): download the latest LTS installer from https://nodejs.org and run it.

Option B — `nvm-windows` (for multiple Node versions):

1. Install `nvm-windows` from https://github.com/coreybutler/nvm-windows/releases or via Chocolatey: `choco install nvm`.
2. Then in an elevated prompt: `nvm install lts` and `nvm use lts`.

After Node.js is installed, install `http-server` if desired:
...
Security note: keep the generated private key (`key.pem`) secure and avoid committing certificates/keys to version control. Add `cert.pem` and `key.pem` to `.gitignore` if you generate them locally.

---

## Technical Architecture

Source: [Technical Architecture](#technical-architecture)
...
Source: [Core Components](#core-components)
...
The full methods description is included below (original source: [METHODS.md](METHODS.md)).
...
Source: [System Requirements](#system-requirements)
...
Source: [Troubleshooting](#troubleshooting)
...
Refer to the [Technical Architecture](#technical-architecture) section for a detailed structure diagram and component descriptions
...
4. Place files where the runtime expects them:
   - `StudentFolders/Experiment/experiment-shapes`
5. Update design declarations:
   - Ensure your shape file names are declared in your manifest (see [IRB-Safe Overview](#irb-safe-overview) and [Overview](#overview) for manifest guidance)
   - If the experiment uses object pairing rules, update exclusions in `objectExclusions.js`
6. Validate before running:
   - Run any available stimulus sanity checks and counterbalancing simulations described in the [IRB-Safe Overview](#irb-safe-overview)
...
Refer to the [Technical Architecture](#technical-architecture) section for a detailed structure diagram and component descriptions (e.g., `index.html`, `trialManager.js`, `webgazer.js`, `localforage.min.js`, and stimulus directories).
...
Source: [IRB-Safe Overview](#irb-safe-overview)
...
Sources: [Overview](#overview), [Technical Architecture](#technical-architecture)
...
- Primary overview and setup: [Overview](#overview)
- Methods: [Methods](#methods)
- IRB-focused overview: [IRB-Safe Overview](#irb-safe-overview)
- Security policy: [SECURITY.md](SECURITY.md)
- Student quick guide: [STUDENT_HANDOUT.md](STUDENT_HANDOUT.md)
- Unit test invariants: [unit_tests.md](unit_tests.md)
- Integrated expanded documentation: [Technical Architecture](#technical-architecture)
