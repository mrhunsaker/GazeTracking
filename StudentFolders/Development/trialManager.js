...
export class TrialManager {
    ...
    async startCalibrationPhase() {
        ...
        // Return a promise that resolves when calibration AND the subsequent experiment complete
            ...
            let currentPointIndex = 0;
                ...
                if (existingPoint) existingPoint.remove();
                    ...
                    if (currentPointIndex >= predefinedPositions.length) {
                        ...
                        // Start experiment and resolve the outer promise only after it completes
                        (async () => {
                            try {
                                if (typeof this.startExperiment === 'function') {
                                    await this.startExperiment();
                                }
                                resolve();
                            } catch (e) {
                                console.error('Error starting experiment after calibration:', e);
                                reject(e);
                            }
                        })();
    ...
    async startExperiment() {
        ...
        try {
            ...
            // Also send to backend to save in data/to_analyze/
    ...
    async saveDataToServer(experimentData) {
        try {
            ...
            // Do not rethrow; data is already saved locally and downloaded
        }
    }
}
