...
def write_markdown_report(output_folder: str, json_file_name: str, types_data: Dict[str, Dict[str, List[float]]], 
    ...
    with open(md_file_path, "w") as md_file:
        ...
        # Wilcoxon Tests
        md_file.write(print_markdown_description("Wilcoxon Test (One-Sided; Target >= 33%)"))
        md_file.write(generate_table(
            headers=["W Statistic", "p-value"],
            data=[[stats["w_test_33"].statistic, format_pvalue(stats["w_test_33"].pvalue)]],
        ))
        
        md_file.write(print_markdown_description("Wilcoxon Test (Two-Sided; Target vs Non-Target)"))
        md_file.write(generate_table(
            headers=["W Statistic", "p-value"],
        ...
        # T-Test
        md_file.write(print_markdown_description("T-Test (Two-Sided; Target vs Non-Target)"))
        md_file.write(generate_table(
