...
while [[ $# -gt 0 ]]; do
    case "$1" in
        -p|--port)
            PORT="$2"; shift 2;;
        -o|--open)
            OPEN_BROWSER=true; shift 1;;
        -h|--help)
            echo "Usage: $0 [-p PORT]"; exit 0;;
        *) echo "Unknown arg: $1"; exit 1;;
    esac
done

ROOT_DIR="$(cd "$(dirname "$0")" && pwd)"
EXP_DIR="$ROOT_DIR/StudentFolders/Experiment"

echo "[runexperiment] Root: $ROOT_DIR"

if command -v node >/dev/null 2>&1; then
    echo "[runexperiment] Running sanity check (node)..."
    node "$EXP_DIR/sanity_check_shapes.js"
else
    echo "[runexperiment] node not found; skipping sanity check. Install Node.js to run checks."
fi

if command -v node >/dev/null 2>&1; then
    if [ -f "$ROOT_DIR/tests/run_unit_tests.js" ]; then
        echo "[runexperiment] Running unit tests..."
        node "$ROOT_DIR/tests/run_unit_tests.js"
    else
        echo "[runexperiment] No unit tests script found at tests/run_unit_tests.js"
    fi
fi

# echo "[runexperiment] Starting static server for $EXP_DIR on port $PORT"
# pushd "$EXP_DIR" >/dev/null
# URL="http://localhost:$PORT/index.html"

# After running tests, show a small action menu: run sanity, run unit tests, or start the full experiment UI
# while true; do
#    echo
#    echo "Runexperiment — choose an action:"
#    echo " 1) Run sanity check"
#    echo " 2) Run unit tests"
#    echo " 3) Start experiment UI & server"
#    echo " 4) Change port (current: $PORT)"
#    echo " 5) Quit"
...
#        1)
#            if command -v node >/dev/null 2>&1; then
#                echo "Running sanity check..."; node "$EXP_DIR/sanity_check_shapes.js" || echo "Sanity check failed";
#            else
#                echo "Node not found; cannot run sanity check.";
#            fi
#            ;;
#        2)
#            if command -v node >/dev/null 2>&1; then
#                if [ -f "$ROOT_DIR/tests/run_unit_tests.js" ]; then
#                    echo "Running unit tests..."; node "$ROOT_DIR/tests/run_unit_tests.js" || echo "Unit tests failed";
#                else
#                    echo "No unit tests script found at tests/run_unit_tests.js";
#                fi
#            else
#                echo "Node not found; cannot run unit tests.";
#            fi
#            ;;
#        3)
#            # Start the experiment UI flow (calls the student interface block)
#            echo "Starting full experiment UI and server..."
#            run_student_interface
#            break
#            ;;
#        4)
#            read -rp $'Enter new port: ' newp; if [[ $newp =~ ^[0-9]+$ ]]; then PORT="$newp"; URL="http://localhost:$PORT/index.html"; echo "Port set to $PORT"; else echo "Invalid port"; fi
#            ;;
#        5)
#            echo "Exiting."; popd >/dev/null; exit 0
#            ;;
#        *) echo "Invalid choice";;
#    esac
#done
...
#run_student_interface() {
    ...
    check_requirements() {
        ...
        if ! command -v flatpak run com.microsoft.Edge  &> /dev/null && ! command -v flatpak run com.google.Chrome &> /dev/null && ! command -v flatpak run org.mozilla.firefox &> /dev/null && ! command -v chromium &> /dev/null; then
    ...
    launch_browser() {
        ...
        elif command -v flatpak run com.microsoft.Edge &> /dev/null; then
            browser_cmd="flatpak run com.microsoft.Edge "

        elif command -v flatpak run org.mozilla.firefox &> /dev/null; then
            browser_cmd="flatpak run org.mozilla.firefox"
